...
**NOTE:** for my specific use case, I needed to have two NordVPN tunnels active at the same time inside my UDM SE, which is not possible with the OpenVPN configuration files provided by NordVPN. In my specific case, after downloading the .conf file generated by the script, I had to modify the IP address in the Address field to prevent overlap between the two configurations. I am still running tests, but everything seems to be working very well.
